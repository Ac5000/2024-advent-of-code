...
class Machine:
    ...
    def solvable(self) -> bool:
        ...
        if self.a_presses > 100 or self.b_presses > 100:
            return False
...
def get_total_cost(machines: list[Machine]) -> int | float:
...
def debug_and_tests():
    ...
    assert machines[0].prize.x == 8400
    assert machines[0].prize.y == 5400
    assert machines[0].a_presses == 80
    assert machines[0].b_presses == 40
    assert machines[0].solvable is True
    assert machines[0].cost_to_win == 280
    assert machines[1].solvable is False
    assert machines[2].a_presses == 38
    assert machines[2].b_presses == 86
    assert machines[2].solvable is True
    assert machines[2].cost_to_win == 200
    assert machines[3].solvable is False
    assert get_total_cost(machines=machines) == 480
