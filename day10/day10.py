...
class Trailhead:
    ...
    def add_trailend(self, coordinate):
        """Increase score if we haven't already found this trailend"""
...
def find_next_path(
    ...
    for path in paths:
        if path is None:
            continue
        # if path.height == 9:
        #     # Found end of trail. Score it and continue.
        #     trailhead.add_trailend(path)
        #     continue
...
def debug_and_tests():
    ...
    assert 0 == grid.sum_trailheads()
    assert 8 == grid.coord_height(0, 0).height
    assert 9 == grid.coord_height(1, 0).height
    assert None is grid.coord_height(0, 8)
    coords = grid.coords_around_coord(Coordinate(1, 1))
    assert 4 == len(coords)
    assert not find_next_path(Coordinate(1, 0, 0), grid, Trailhead(2, 0))

    # Simple 2 score single trailhead example
    topo_map = get_input("day10example1")
    grid = Grid(initial_map=topo_map)
    assert 1 == len(grid.trailheads)
    chart_path(grid.trailheads[0], grid)
    assert 2 == grid.sum_trailheads()

    # 4 score single trailhead example
    topo_map = get_input("day10example2")
    grid = Grid(initial_map=topo_map)
    assert 1 == len(grid.trailheads)
    chart_path(grid.trailheads[0], grid)
    assert 4 == grid.sum_trailheads()

    # 3 score double trailhead example
    topo_map = get_input("day10example3")
    grid = Grid(initial_map=topo_map)
    assert 2 == len(grid.trailheads)
    for trailhead in grid.trailheads:
        chart_path(trailhead, grid)
    assert 3 == grid.sum_trailheads()

    # Main example, 9 trailheads, 36 score
    topo_map = get_input("day10example")
    grid = Grid(initial_map=topo_map)
    assert 9 == len(grid.trailheads)
    for trailhead in grid.trailheads:
        chart_path(trailhead, grid)
    assert 36 == grid.sum_trailheads()
...
def main():
    ...
    for i, trailhead in enumerate(grid.trailheads):
        print(f"Trailblazing {i}/{len(grid.trailheads)}")
        chart_path(trailhead, grid)
    grid.sum_trailheads()
